...
void testTurn(void) {
    ...
    while (turn < 25) {
        //at this point, we're at the start of a new game,
        //so the current turn is 0, the num of turn is 1
        //turn 0, player 0; turn 1, player 1...
        printf ("Testing that the current turn is %d\n", turn);
        assert(currentTurn(g) == turn);
        printf ("Testing that the number of turn is %d\n", turn + 1);
        ...
        //now we end the 0 turn, so current turn becomes 1
        //the num of turn becomes 2
        //which will be tested in the beginning of this while loop
        turn++;
    }
}
...
//test playerDirection, by constracting a game
//making moves as each player
//check that if the direction is correct
void testPlayDirection(void) {
    //all has the same type of suit
    //Red 1 HEARTS;
    //easiest way
    ...
    Game g = sameCards(void);
    //at this point, we're at the start of a new game,
    //cannot determine the direction yet
    //the current player is 0
    //Now make move as the first player(player 0)
    //0->1->2->3->0
    int i = 0;
    while (i < 25) {
        //all the same
        ...
        finalMove.action = END_TURN;
        playMove (g, finalMove);
        //player (i) to player(i+1)
        //so the direction is clockwise
        printf("Testing the direction is always clockwise\n");
...
*/
    ...
    //end 2ed turn
    playMove (g, finalMove);
    //current player:player 2. So far, is clockwise
    assert(playDirection(g) == CLOCKWISE);
    ...
    playMove (g, finalMove);
    //player 1, because of player 2 discard "BACKWARDS"
    //direction reversed, becomes ANTICLOCKWISE
    assert(playDirection(g) == ANTICLOCKWISE);

    //4th turn - 9 turns
    //continue playing cards not value "BACKWARDS"
    int turn = 4;
    while (turn < 10) {
        move = playCard(g, 0);
        playMove(g, move);
    ...
    //10th turns
    //play "BACKWARDS" card again, direction becomes clockwise again
    move = playCard(g, 7);
    playMove(g, move);
    //end 10 turn
    playMove (g, finalMove);
...
// by constructing a very simple game
// making moves as each player, and checking that the
// the number of cards in each player's hand is correct
void testPlayerCardCount(void) {
    //for deck made up of same cards
    Game g = sameCards(void);
    //at begining, each player has 7 cards
    int player = 0;
    while (player < 4) {
        assert(playerCardCount(g, player) == 7);
        player++;
    }

    //player 0 discard one card
    playerMove move = playCard(g, 0);
    playMove(g, move);
    playerMove finalMove;
    finalMove.action = END_TURN;
    ...
    assert(playerCardCount(g, 1) == 7);
    assert(playerCardCount(g, 2) == 7);
    assert(playerCardCount(g, 3) == 7);
...
//by constructing a very simple game
//making moves as each player, and checking that the
//the number of cards in current player's hand is correct
void testhandCardCount(void) {
    Game g = sameCards(void);
    //turn 0, player 0 has 7 cards
    //turn 1, player 1 has 7 cards
    //...
    //turn 4, player 0 has 6 cards
    //turn 5, player 1 has 6 cards
    //...

    int i = 0;
    while (i < 4) {
        assert(handCardCount(g) == 7);
        //play the move = play a card
        playerMove move = playCard(g, 0);
        playMove(g, move);
        playerMove finalMove;
        finalMove.action = END_TURN;
    ...
    assert(handCardCount(g) == 5);
}

void testHandCard(void) {
    //test 1, using deck have the same cards
    Game g = sameCards(void);
    Card new = newCard(1, RED, HEARTS);
    assert(handCard(g, 2) == new);
    assert(handCard(g, 3) == new);
    ...
    finalMove.action = END_TURN;
    playMove (g, finalMove);
    assert(handCard(g, 0) == new);
    assert(handCard(g, 4) == new);

    //test 2, using deck have two types of cards
    destroyGame(g);
    g = deck1(void);
    new = newCard(1, YELLOW, CLUBS);
    Card news = newCard(BACKWARDS, YELLOW, CLUBS);
    //1st card in player 0 hand is Yellow, 1, CLUBS
    assert(handCard(g, 0) == new);
    //6th card in player 0 hand is Yellow, BACKWARDS, CLUBS
    assert(handCard(g, 7) == news);
    ...
    playMove (g, finalMove);
    assert(handCard(g, 0) == new);
    assert(handCard(g, 5) == news);
    assert(handCard(g, 6) == news);
}
